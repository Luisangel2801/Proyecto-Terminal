...
\section{Desarrollo del Proyecto}
    ...
    El sensor de fotopletismografía (PPG) es un método no invasivo para medir la frecuencia cardíaca y la saturación de oxígeno en la sangre. Debido a que su salida es una señal analógica susceptible a ruido eléctrico y artefactos de movimiento, es necesario incorporar filtros. Para ello, se implementaron filtros activos de segundo orden tipo \textit{Sallen-Key} para eliminar, respectivamente, el ruido de alta y baja frecuencia, junto con un filtro pasa bandas destinado a suprimir la interferencia de 60 Hz.
        ...
        Finalmente, para ajustar la señal al rango de voltaje adecuado para el puerto ADC del ESP32, se utilizó un amplificador inversor con fuente única. Este amplificador permite ajustar la señal de PPG, de modo que oscile entre 0 y 3.3 V, lo que resulta ideal para el ADC del ESP32. La ganancia del amplificador inversor se calcula utilizando la ecuación~\ref{eq:ganancia_amplificador_inversor_ppg}.
        ...
        Primero, amplificaremos la señal filtrada del PPG para mejorar su visualización. Para ello, utilizaremos los siguientes valores de resistencia: $R2 = 100k\Omega$ y $R1 = 1k\Omega$. La ganancia del amplificador inversor se calcula de la siguiente manera:
        ...
        Lo que significa que la señal de salida será 100 veces mayor que la señal de entrada. Sin embargo, como el ESP32 tiene un rango de voltaje de 0 a 3.3 V, es necesario ajustar la señal para que oscile entre 0 y 3.3 V. Para agregar un desplazamiento de 1.5 V a la señal, se utiliza un divisor de voltaje con dos resistencias de $56 K\Omega $ y $47 K\Omega$. La salida del divisor de voltaje se conecta a la entrada no inversora del amplificador operacional, lo que permite desplazar la señal de salida a un rango de 0 a 3.3 V se utiliza la siguiente ecuación~\ref{eq:desplazamiento_amplificador_inversor_ppg}.

        \begin{equation}
            \label{eq:desplazamiento_amplificador_inversor_ppg}
            V_{offset} = \frac{Rb}{Ra + Rb} \cdot VCC
        \end{equation}

        Donde $V_{offset}$ es el voltaje de desplazamiento, $R_a = 56k\Omega$ y $R_b = 47k\Omega$ y $VCC$ es el voltaje de alimentación del amplificador operacional, que en este caso es de 3.3 V. Sustituyendo los valores en la ecuación~\ref{eq:desplazamiento_amplificador_inversor_ppg}, se obtiene el valor de $V_{offset}$.

        \begin{equation}
            V_{offset} = \frac{47k\Omega}{56k\Omega + 47k\Omega} \cdot 3.3 V = \frac{47k\Omega}{103k\Omega} \cdot 3.3 V = 0.456 \cdot 3.3 V = 1.5058 V
        \end{equation}

        El valor de $V_{offset}$ es aproximadamente 1.5 V, lo que provoca que la señal de salida se desplace a 1.5 V. Esto permite que la señal oscile entre 0 y 3.3 V, lo cual es adecuado para el ADC del ESP32.
        
        Para obtener el capacitor $C_1$ se utiliza la siguiente ecuación~\ref{eq:capacitancia_C1_amplificador_inversor_ppg}.
        \begin{equation}
